...
def build_selectors_by_intent(min_example: int = 3) -> dict[str, SemanticSimilarityExampleSelector]:
    ...
    selectors = {}
    for intent, exlist in grouped.items():
        if len(exlist) >= min_example:
            selectors[intent] = build_selector(exlist)
        else:
            print(f"⚠️ Intent '{intent}' hanya punya {len(exlist)} contoh. Akan fallback ke 'general'")
            # Jangan buat selector kalau terlalu sedikit, akan fallback ke 'general'
            continue

    # Pastikan ada fallback 'general'
    if "general" not in selectors:
        selectors["general"] = build_selector(grouped["general"])

    return selectors

def get_prompt_by_intent(intent: str, selectors: dict) -> FewShotPromptTemplate:
    # Gunakan fallback ke 'general' jika intent tidak ditemukan
    selector = selectors.get(intent) or selectors.get("general")
    return make_prompt(selector)
